...
import BuildTypes, { BuildType } from '../../../components/BuildTypes';
...
const Types: NextPage = () => {
    ...
    const [buildTypesList, setBuildTypes] = useState<BuildType[]>([]);
    ...
    useEffect(() => {
        ...
        handleSelectedMenu('services-builds');
        ...
        if (user) {
            if (can(user, "services", "view")) {
                ...
                api.get('build-types').then(res => {
                    ...
                    console.log('Error to get builds, ', err);
    ...
    async function handleBuildTypesList() {
        const res = await api.get('build-types');
    ...
    function handleOnDragEnd(result: DropResult) {
        if (result.destination) {
            ...
            const updatedListDocs = produce(buildTypesList, draft => {
                if (draft) {
            ...
            if (updatedListDocs) {
                setBuildTypes(updatedListDocs);
                saveOrder(updatedListDocs);
            }
        }
    }

    async function saveOrder(list: BuildType[]) {
        list.forEach(async (doc, index) => {
            try {
                await api.put(`build-types/${doc.id}`, {
                    name: doc.name,
                    order: index
                });

                handleBuildTypesList();
            }
            catch (err) {
    ...
    return (
        <>
            <NextSeo
                ...
                description={`Tipos de estabelecimentos da plataforma de gerenciamento da ${process.env.NEXT_PUBLIC_STORE_NAME}.`}
                openGraph={{
                    ...
                    images: [
                        {
                            ...
                            alt: `Tipos de estabelecimentos | Plataforma ${process.env.NEXT_PUBLIC_STORE_NAME}.`,
                        },
                        { url: `${process.env.NEXT_PUBLIC_APP_URL}/assets/images/logo.jpg` },
                    ],
                }}
                                                            ...
                                                            <Droppable droppableId="lines">
                                                                {provided => (
                                                                    ...
                                                                    >
                                                                        <ListGroup>
                                                                            {
                                                                                buildTypesList && buildTypesList.map((buildType, index) => {
                                                                                    return <Draggable key={buildType.id} draggableId={buildType.id} index={index}>
                                                                                        {(provided) => (
                                                                                            <div
                                                                                            ...
                                                                                            >
                                                                                                <BuildTypes
                                                                                                    buildType={buildType}
                                                                                                    buildTypesList={buildTypesList}
                                                                                                    handleBuildTypesList={handleBuildTypesList}
                                                                                                />
                                                                                            </div>
                                                                                        )}
                                                        ...
                                                        <Col>
                                                            <Row>
                                                                <Col className="text-center">
                                                                    <p style={{ color: 'var(--gray)' }}>Nenhum tipo de estabelecimento registrado.</p>
                                                                </Col>
                                                            </Row>
                                ...
                                <Modal show={showModalNewType} onHide={handleCloseModalType}>
                                    ...
                                    <Formik
                                        ...
                                        onSubmit={async values => {
                                            ...
                                            try {
                                                if (buildTypesList) {
                                                    await api.post('build-types', {
                                                        name: values.name,
                                                        active: values.active,
                                                        order: buildTypesList.length,
                                                    });

                                                    await handleBuildTypesList();
                                                ...
                                                console.log('error create build type.');
                                                console.log(err);
                                            }
