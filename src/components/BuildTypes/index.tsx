...
interface BuildTypesProps {
    buildType: BuildType;
    buildTypesList: BuildType[];
    handleBuildTypesList(): Promise<void>;
}
...
const BuildTypes: React.FC<BuildTypesProps> = ({ buildType, buildTypesList, handleBuildTypesList }) => {
    ...
    async function deleteLine() {
        ...
        try {
            await api.delete(`build-types/${buildType.id}`);

            const list = buildTypesList.filter(item => { return item.id !== buildType.id });

            list.forEach(async (buildType, index) => {
                try {
                    await api.put(`build-types/${buildType.id}`, {
                        name: buildType.name,
                        order: index
                    });
                }
                catch (err) {
                    console.log('error to save build types order after deleting.');
                    console.log(err)
                }
            });
            ...
            console.log("Error to delete build type");
            console.log(err);
        }
    }
    ...
    return (
        <ListGroup.Item variant="light">
            <Row className="align-items-center">
                ...
                <Col><span>{buildType.name}</span></Col>
                ...
                <Formik
                    ...
                    onSubmit={async values => {
                        ...
                        try {
                            ...
                            console.log('error edit build type.');
                            console.log(err);
